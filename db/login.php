...
if (isset($_POST['uname']) && isset($_POST['password'])) {
	...
	if (empty($uname)) {
		header("Location: loginDummy.php?error=User Name is required");
	    exit();
	}else if(empty($pass)){
        header("Location: loginDummy.php?error=Password is required");
	    exit();
	}else{
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['Username'] === $uname && $row['Password1'] === $pass) {
				...
				header("Location: loginDummy.php?error=Incorect User name or password");
		        exit();
			}
		}else{
			header("Location: loginDummy.php?error=Incorect User name or password");
